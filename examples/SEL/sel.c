...
static void push_args_array(vud_rank *r, dpu_arguments_t *args, uint32_t nr_of_dpus) {
    ...
    _Alignas(8) uint64_t staged[NR_DPUS][4];
...
int main(int argc, char **argv) {
    ...
    const uint32_t input_size = (p.exp == 0) ? (p.input_size * nr_of_dpus) : p.input_size;
    const uint32_t input_size_dpu_ = divceil(input_size, nr_of_dpus);
    const uint32_t input_size_dpu_round = (input_size_dpu_ % (NR_TASKLETS * REGS) != 0)
                                            ? roundup(input_size_dpu_, (NR_TASKLETS * REGS))
                                            : input_size_dpu_;
    const size_t bytes_per_dpu = (size_t)input_size_dpu_round * sizeof(T);
    const size_t total_bytes   = (size_t)input_size_dpu_round * sizeof(T) * nr_of_dpus;
    ...
    for(int rep = 0; rep < p.n_warmup + p.n_reps; rep++) {
        ...
        cpu_total = select_host(C, A, input_size);
        if(rep >= p.n_warmup)
            stop(&timer, 0);
        ...
        if (r.err) { fprintf(stderr, "args0 transfer failed: %s\n", vud_error_str(r.err)); return EXIT_FAILURE; }

        /* Push A */
        {
            const vud_mram_size wordsA = (vud_mram_size)((bytes_per_dpu + 7u) / 8u);
            const uint64_t *lanes[NR_DPUS];
            for (uint32_t d = 0; d < nr_of_dpus; ++d) lanes[d] = (const uint64_t*)(A + (size_t)d * input_size_dpu_round);
...

        {
            ...
            for (uint32_t i = 0; i < nr_of_dpus; ++i)
                dsts[i] = (uint64_t*)(slab + (size_t)i * input_size_dpu_round);
        ...
        for (uint32_t d = 0; d < nr_of_dpus; ++d) {
            ...
            if (tcnt) {
                T *src = slab + (size_t)d * input_size_dpu_round;
                T *dst = C2  + (size_t)per_dpu_offsets[d];
                memcpy(dst, src, (size_t)tcnt * sizeof(T));
            }
        }
        free(slab);
        if(rep >= p.n_warmup)
